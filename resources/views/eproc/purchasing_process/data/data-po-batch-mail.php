...
$querypo = "SELECT p.*, v.nm_vendor, u.id_user, u.username, u.status_user from purch_proc.po_list p 
			  ...
			  and u.status_user = 'A' 
              ";
                //order by u.username REGEXP '^[^@]+@[^@]+\.[^@]{2,}$' desc
                //group by p.po_num
                //or ( p.batch = (select max(p.batch) from purch_proc.po_list) and p.sent = '0000-00-00 00:00:00')
...
if(mysqli_num_rows($query_exec) > 0 ){
    ...
    while($row = mysqli_fetch_array($query_exec)){
        ...
        $upload_date = date("d.m.Y H:i:s", $dateformat_upl);
