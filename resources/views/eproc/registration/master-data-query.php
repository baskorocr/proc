...
function get_vendor_login($username, $pass)
{
	...
	$result = array();
	$vendor = DB::collection('vendor')
				->where('id_vendor',$user['foreign_id'])
				->first();
	
	if($vendor) {
		$user_vendor = DB::collection('user')
				->where('id_user',$user['id_user'])
				->first();
	
		$tipe_user = DB::collection('tipe_user')
					->where('id_tipe_user',$user['id_tipe_user'])
					->first();			
		
		$result = array_merge($user_vendor,$tipe_user,$vendor);
	}
...
function get_user_login($username, $pass)
{
	...
	$result = array_merge($user_login,$tipe_user);
...
function get_menu_group_by_id_user($id_user)
{
	...
	$user = DB::table('user')->where('id_user',$id_user)->where('status_user','A')->first();
	
	$access_group_list = DB::table('access_group_list')
							->where('id_access_group',$user['id_access_group'])
							->first();
	
	$menu_access_group = DB::table('menu_access_group')
						->where('id_access_group',$access_group_list['id_access_group'])
						->get();
	
	$ids_menu = array();
	foreach($menu_access_group as $mag){
		array_push($ids_menu,$mag['id_menu']);
	}
	
	$menu_group = DB::table('menu_group')
					->wherein('id_menu',$ids_menu)
					->groupBy('id_menu_group')
					->orderBy('id_menu_group')
					->get();
	
	$ids_menu_group = array();
	foreach($menu_group as $mg){
		array_push($ids_menu_group,$mg['id_menu_group']);
	}
	
	$menu_group_list = DB::table('menu_group_list')
						->whereIn('id_menu_group',$ids_menu_group)
						->orderBy('id_menu_group')
						->get();
	
	//var_dump($menu_group_list);die;
	
	return $menu_group_list;
...
{
	$user = Auth::user();
	$menu_access_group = DB::collection('menu_access_group')
							->where('id_access_group',$user->id_access_group)
							->get();
	
	$id_menu_arr = array();
	foreach($menu_access_group as $mag) {
		array_push($id_menu_arr, $mag['id_menu']);
	}
	
	$menu_list = DB::collection('menu_list')
					->join('menu_group','menu_group.id_menu','=','menu_list.id_menu')
					->whereIn('id_menu', $id_menu_arr)
					->orderBy('menu_group.id_menu_group', 'Asc')
					->get()->toArray();
					
	$result = array_merge($menu_list);
