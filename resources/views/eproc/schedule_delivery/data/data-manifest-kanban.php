...
$querymf = "SELECT h.manifest, d.kanban, h.delivery_date, h.po_num, h.id_vendor, h.stat, d.active, v.nm_vendor, d.material, d.material_desc, sum(d.qty_pack) as qty_tot, 
              count(d.kanban) as tot_kanban, SUM(d.qty_in) as tot_qty_in, qty_pack, d.arrival_date, d.arrival_time,
              (select count(dd.issued_date) from manifest_detail dd where dd.kanban = d.kanban and dd.issued_date <> '0000-00-00' 
              group by dd.kanban) as received_kanban 
    ...
    while($row = mysqli_fetch_array($query_exec)){
        ...
        */
        ...
        if ($tot_kanban == $kanban_received) {
            $receive_stat = "<small><h4><span class=\"badge bg-green\">done</span></h4></small>";
        } else {
            $receive_stat = "<small><h4><span class=\"badge bg-orange\">waiting</span></h4></small>";
        }
        ...
        if($row['arrival_date'] != '0000-00-00'){
            $dateArrival = strtotime($row['arrival_date']);
            $timeArrival = strtotime($row['arrival_time']);
            $arrival_date = date("D, d.m.Y", $dateArrival)." ".date("H:i", $timeArrival);
            $scan_stat = "<small><h4><span class=\"badge bg-green\">Done</span></h4></small>";
        } else {
            $arrival_date = '-';
            $scan_stat = "<small><h4><span class=\"badge bg-orange\">Waiting</span></h4></small>";
