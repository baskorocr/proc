...
<div class="box-header">
    <h3 class="box-title">Dashboard Monitoring Projects</h3>   
</div><!-- /.box-header -->
<hr style="margin-top: 1px;">
<?php 
/*
if(in_array('7', $_SESSION['access_menu'])){
    ...
    <table id="example2" class="table table-bordered table-striped">                      
        ...
        <tbody> 
           ...
           <?php
                ...
                $query_exec = dashboard_proc_data();
                ...
                while ($row = mysqli_fetch_assoc($query_exec)) {
                    ...
                    if ($id_vendor != "" ){
                        $vendor_ass = "<span>".$row['nm_vendor']."</span>";
                    } else {
                        $vendor_ass = "<h4><span class='badge bg-yellow'>Waiting...</span></h4>";
                    }
                    ...
                    if ($count > 1 ){
                        $project_type = "<h4><span class='badge bg-blue'>Multisrc</span></h4>";
                    } elseif ($count == 1) {
                        $project_type = "<h4><span class='badge bg-green'>Single Vdr</span></h4>";
                    } else {
                        $project_type = "<h4><span class='badge bg-red'>UNDEFINED</span></h4";
                    }
                    ...
                    //count uploaded doc by engineering and make stat uploaded doc
                    $query_exec_new1 = get_proj_doc_assign_data_status_all_vendor($id_project, $id_vendor);
                    //$row_new = mysqli_fetch_assoc($query_exec_new1);
                    $row_new = mysqli_fetch_assoc($query_exec_new1);

                    $query_exec_new2 = get_proj_doc_assign_data_status_sum_vendor($id_project, $id_vendor);
                    //$row_new2 = mysqli_fetch_assoc($query_exec_new2);
                    $row_new2 = mysqli_fetch_assoc($query_exec_new2);

                    //$count_row_new = mysqli_num_rows($query_exec_new1);
                    $count_row_new = mysqli_num_rows($query_exec_new1);
                    $sum_check_new = $row_new2['check_status'];

                    if ($count_row_new == $sum_check_new AND ($sum_check_new != "" AND $count_row_new != "") ){
                        $stat_check_new =  "<h4><span class='badge bg-green'>Completed ($sum_check_new/$count_row_new)</span></h4>";
                        $checked_doc_eng_vend = 1; //ok
                    } elseif ($count_row_new != $sum_check_new AND ($count_row_new != 0 AND $sum_check_new != 0)) {
                        $stat_check_new =  "<h4><span class='badge bg-yellow'>Uncomplete ($sum_check_new/$count_row_new)</span></h4>";
                        $checked_doc_eng_vend = 0; //not ok
                    } else {
                        $sum_check_new = 0;
                        $stat_check_new =  "<h4><span class='badge bg-red'>Unchecked ($sum_check_new/$count_row_new)</span></h4>";
                        $checked_doc_eng_vend = 0; //not ok
                    }
                    ...
                    if ( $count_row4 == $count_row5 AND ($count_row4 != 0 AND $count_row5 != 0) ){
                        ...
                        $upload_stat =  "<h4><span class='badge bg-red'>Uncomplete ($count_row4/$count_row5)</span></h4>";
                    }
                    */

                    if ( $count_row4 == $count_row5 AND ($count_row4 != 0 AND $count_row5 != 0) ){
                        $upload_stat =  "<h4><span class='badge bg-green'>Completed ($count_row5/$count_row4)</span></h4>";
                        $checked_upload_eng = 1;
                    } elseif ($count_row4 != $count_row5 AND ($count_row4 > 0 AND $count_row5 != 0)) {
                        $upload_stat =  "<h4><span class='badge bg-yellow'>Uncomplete ($count_row5/$count_row4)</span></h4>";
                        $checked_upload_eng = 0;
                    } else {
                        $upload_stat =  "<h4><span class='badge bg-red'>No upload($count_row5/$count_row4)</span></h4>";
                        $checked_upload_eng = 0;
                    }
        ...
        if ($doc_assignment == 1 AND $checked_doc_eng_vend == 1 AND $checked_upload_vdr == 1 AND $checked_fin_proc == 1 AND $checked_fin_qa == 1){
            ...
            ?>
                <tr>
                    <td><small><?php echo $row['proj_num'];?></small></td>
                    ...
                    <td><?php echo $stat_check_new; ?></td>

                    <td><small><?php echo $vendor_ass;?></small></td>
                    <td><?php echo $upload_vendor_stat; ?></td>
                    <td><?php echo $stat_check_fin_proc; ?></td>
                    <td><?php echo $stat_check_fin_qa; ?></td>
                    <!--
                    <td><small><?php echo $project_type; ?></small></td>
                    -->
