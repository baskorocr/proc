...
<div class="box-header">
	<h3 class="box-title">Upload Email Vendor</h3>
</div>

    <form role=form name="myForm" id="myForm" onSubmit="return validateForm()" action="home.php?mnu=vdremailupdate" method="post" enctype="multipart/form-data">
        
        <div class="box-body">                            
            <div class="form-group">
               	<label for="exampleInputFile">Upload Email Vendor</label>
            ...
            <br>
                ...
                //    import data excel mulai baris ke-6 (karena tabel xls hasil export dari SAP)
                    for ($i=2; $i<=$baris; $i++)
                    {
                        ...
                        $foreign_id        	= $data->val($i, 1); //ID VENDOR
                        $email        		= strtoupper($data->val($i, 2));
                        $password           = strtoupper($data->val($i, 3));
						$stat 				= 'A';
                        ...
                        //if ($id_vendor != $row['id_vendor']){
                            update_email_vendor_data($foreign_id, $email, $password, $stat);
