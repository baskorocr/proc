...
if(isset($_POST['selectmf'])) {
        ...
        if (count($filemf) >= 1) {
            ...
            $filename_zip = "DHARMA_POLIMETAL_MFPDF_".date("d-m-Y").".zip";
            
            /******  START GET MANIFEST FILE *******/

            //$rootPath = realpath('DATA');
            //$rootPath = "\\\\sysdata2\\MANIFEST\\".$mf_type."\\";   
			//$rootPath = "\\\\sysdata2\\MANIFEST\\".$mf_type."\\PRD-".$mf_type."\\";  
			$rootPath = "D:\\\\MANIFEST\\".$mf_type."\\PRD-".$mf_type."\\";  
			...
			//$rootPath2 = "\\\\sysdata2\\MANIFEST\\".$kanban_folder."\\PRD-".$kanban_folder."\\";  
			$rootPath2 = "D:\\\\MANIFEST\\".$kanban_folder."\\PRD-".$kanban_folder."\\";  
                    ...
                    for ($i = 0; $i < $ct_filemf; $i++) {
                        ...
                        //if ($file->getFilename() == $mf_arr[$i] ) { //$filemf[$i]
                            ...
                            if(update_mf_download_mail($data)) {
								...
								$zip->addFile($rootPath."/".$mf_arr[$i], "01 Manifest/".$mf_arr[$i]);
                            ...
                            for ($i = 0; $i < $ct_filekanban; $i++) {
                                ...
                                //if ($file2->getFilename() == $filekanban[$i]) {
                                    ...
                                    if(update_mf_download_mail($data)) {
										...
										$zip->addFile($rootPath2."/".$filekanban[$i], "02 Kanban/".$filekanban[$i]);
                                        //rename file
