...
class PoController extends Controller
{
    ...
    public function sendPoMultiple(Request $request)
    {
        ...
        foreach ($request->data as $value) {
            $user = User::where('foreign_id', $value['id_vendor'])->where('is_vendor', true)->where('status_user', 'A')->get();
            $cekPo = Po::where('po_num', $value['po_num'])->where('revno', $value['revno'])->count();
